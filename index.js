...
const BOT_OWNER_ID = '774278713684394004'; // ✅ YOUR ID
...
const MESSAGE_EXP = 1;
const BUMP_EXP = 5;
const MAX_INVENTORY = 100; // ✅ Inventory limit
...
// ✅ FIXED: Atomic message EXP with proper cooldown
...
// ✅ IMPROVED: Better cooldown formatting
...
client.on('ready', async () => {
  ...
  try {
    ...
    if (missing.length > 0) {
      console.warn(`⚠️ Missing permissions in gacha channel: ${missing.join(', ')}`);
    }
...
client.on('messageCreate', async (message) => {
  try {
    ...
    if (cmd.startsWith('!owner')) {
      ...
      if (ownerCmd === 'refreshpanel') {
        try {
          ...
          if (!channel) {
            return sendCommandReply(
              new EmbedBuilder().setColor('#ff7675').setDescription('❌ Gacha channel not found.')
            );
          }
          ...
          await setState('gacha_panel_id', msg.id);
          return sendCommandReply(
            new EmbedBuilder().setColor('#00b894').setDescription('✅ Gacha panel refreshed successfully!')
          );
        } catch (err) {
          return sendCommandReply(
            new EmbedBuilder().setColor('#ff7675').setDescription(`❌ Error: ${err.message}`)
          );
        }
      }
        ...
        try {
          ...
          return sendCommandReply(
            new EmbedBuilder()
              .setColor('#00b894')
              .setDescription(
                `✅ All users' EXP has been reset to 0.\n` +
                `📊 Updated: **${usersUpdated}** user(s)\n` +
                `*Inventory, luck, and cooldowns remain unchanged.*`
              )
          );
        } catch (err) {
          return sendCommandReply(
    ...
    if (cmd === '!gacha') {
      return sendCommandReply(buildGachaPanel());
    }

    if (cmd === '!profile') {
      const bumpStatus =
        now - user.lastBump >= BUMP_COOLDOWN ? '🟢 Ready' : `⏳ ${formatCooldown(BUMP_COOLDOWN - (now - user.lastBump))}`;
      const dailyStatus =
        now - user.lastDaily >= DAILY_COOLDOWN ? '🟢 Ready' : `⏳ ${formatCooldown(DAILY_COOLDOWN - (now - user.lastDaily))}`;
    ...
    if (cmd === '!leaderboard') {
      ...
      const PAGE_SIZE = 10;
      const top10 = rows.slice(0, PAGE_SIZE);
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `RENMA SYSTEM  •  ${rows.length} total players` })
        .setTimestamp();
      return sendCommandReply(embed);
    }
    ...
    if (cmd === '!inventory') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `${user.inv.length}/${MAX_INVENTORY} items  •  RENMA SYSTEM` });
      return sendCommandReply(embed);
    }
  } catch (err) {
        ...
        return handleEphemeral({
          embeds: [
            new EmbedBuilder()
              .setColor('#ff7675')
              .setDescription(
                `❌ Inventory full! Max **${MAX_INVENTORY}** items.\n\n` +
                `Drop some items or use them to make space.`
              )
